...
```text
figure/   测试集 ROC、校准、DCA 图，以及 SHAP 解释图
results/  四种模型交叉验证比较结果 CSV
```
...
7. **测试集评价**：ROC-AUC（2000 次 bootstrap 95% CI）、Sensitivity / Specificity / PPV / NPV、混淆矩阵、Brier score、校准截距与斜率、决策曲线分析（DCA）
8. **可解释性**：用 SHAP `LinearExplainer` 解释最终模型，生成全局重要性、方向性 beeswarm 和单样本 waterfall 图
...
- 可进一步增加：概率校准方法比较、重复交叉验证、更大独立验证集
